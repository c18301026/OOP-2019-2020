...
public class PitchSpeller {
	...
	public String spell(float frequency) {
		float difference = Math.abs(frequencies[0] - frequency);
		String returnValue = spellings[0];

		for(int i = 0; i < frequencies.length; i++) {
			float newDifference = Math.abs(frequencies[i] - frequency);

			if(newDifference < difference) {
				difference = newDifference;
				returnValue = spellings[i];
			}
		}
